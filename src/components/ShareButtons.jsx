...
const ShareButtons = ({ mbtiType, character }) => {
  ...
  const handleKakaoShare = async () => {
    ...
    if (!window.Kakao || !window.Kakao.isInitialized()) {
      ...
      alert(t('shareButtons.kakaoError'));
      return;
    }
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-center gap-3 mb-2">
        ...
        <h4 className="text-gray-400 text-xs font-medium">{t('shareButtons.shareTitle')}</h4>
        <div className="h-px bg-gray-700 flex-1"></div>
      </div>
      ...
      <div className="grid gap-3">
        ...
        >
          <span className="flex items-center justify-center gap-3">
            ...
            {isSharing ? t('shareButtons.sharing') : t('shareButtons.kakao')}
          </span>
        </button>
      ...
      <div className="mt-4 pt-4 border-t border-gray-700">
        <div className="flex items-center justify-center gap-6 text-xs">
          <div className="flex items-center gap-2 text-gray-500">
            ...
            <span>{t('shareButtons.compareWithFriends')}</span>
          </div>
        </div>
      </div>
    </div>
